...
//* ---------------------------------------------
//* Step 3 Find overlaps SNPs between .bims and .bed
//*----------------------------------------------

overlap_script = '/path/to/overlap.py'
overlap_bed_storage = '/path/to/put/the/overlap/bed/files'
//these .bed files have a forth column which states the number SNPs that falls in each peak 

process findOverlaps {
    echo true 
    conda '/rds/general/user/xyz16/home/anaconda3/envs/py37'
    label 'small'
    publishDir "overlap_bed_storage", pattern: 'overlaps_*.bed', mode: 'copy'
    ...
    output: 
    path "overlaps_*.bed" into overlapbedbeds

    script: 
    """ 
    python $overlap_script $bimpath $liftedbed
    """
}

//*-------------------------------------------
//* Step 4 Select the most specific 0.2% SNPs 
//*-------------------------------------------

final_bed = '/path/to/store/the/bed/files/that/contains/peaks/which/covers/0.2%SNPs'
top_select = '/path/to/top_select.py'

process seclecTop{
    echo true 
    conda '/rds/general/user/xyz16/home/anaconda3/envs/py37'
    label 'small'
    publishDir "final_bed", pattern: '*.bed', mode: 'copy'

    input: 
    path overlapbedbeds 

    output: 
    path "*.bed" into final_bed

    script: 
    """ 
    python $top_select $overlapbedbeds
    """
...
//*---------------------------------------------------
//* Step 5 Create .annot files for S-LDSC
//*--------------------------------------------------
process makeannot {
    echo true
label 'singularity'
...
publishDir "$Annot_LD", pattern: '*.annot.gz', mode: 'copy'
    ...
    script:
    bed = final_bed.name.replace(".bed", "").replace("overlap_", "")
...
//check the names of the chr, because this could be outdaated if plink files update in the future 
    ...
    """
    python $op1 --bed-file $final_bed --bimfile $bim --annot-file "${bed}.${chr}.annot.gz"
    """
}

/*---------------------------------------------------
//* Step 6 Create LD files for S-LDSC
//*--------------------------------------------------
...
//***-----------------------------------------------------
//*** Step 7 Patrition heritability into different cell types  
//***-----------------------------------------------------
...
//*---------------------------------------------
//* Step 8 Python function for organizing the LDSC results
//*---------------------------------------------
...
//*** --------------------
//* Step 10 R Plot for S-LDSC results  
//*-----------------------
